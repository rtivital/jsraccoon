# Frontend Raccoon
Репозиторий группы [Frontend Raccoon](http://vk.com/jsraccoon). В репозиторий регулярно добавляются советы по верстке и написанию JavaScript кода, а также небольшие заметки о тонкостях JavaScript.

## Содержание

### Статьи
* [Перебирающие методы массивов изнутри](/articles/fn-1-array-methods.md)
* [Продвинутое использование метода reduce](/articles/fn-2-advanced-reduce.md)
* [Стилизация плэйсхолдеров с препроцессором Sass](/articles/css-1-placeholders.md)

#### Объектно-ориентированный JavaScript
* [Объектно-ориентированный JavaScript: примитивы и объекты](/articles/oop-1-objects-vs-primitives.md)
* [Объектно-ориентированный JavaScript: работа с объектами](/articles/oop-2-object-base.md)
* [Объектно-ориентированный JavaScript: функции конструкторы](/articles/oop-3-constructors.md)
* [Объектно-ориентированный JavaScript: прототипы](/articles/oop-4-prototypes.md)
* [Объектно-ориентированный JavaScript: немного практики](/articles/oop-5-example-validation.md)
* [Объектно-ориентированный JavaScript: наследование](/articles/oop-6-inheritance.md)
* [Объектно-ориентированный JavaScript: наследование на практике](/articles/oop-7-inheritance-practice.md)
* [Объектно-ориентированный JavaScript: дескрипторы](/articles/oop-8-descriptors.md)

#### ES6
* [Перед тем, как вы начнете](/articles/es6-1-before-you-start.md)
* [Вместо введения](/articles/es6-2-introduction.md)
* [Блочные области видимости, let, const, временная мертвая зона](/articles/es6-3-block-scoped-declarations.md)
* [Операторы spread и rest](/articles/es6-5-spread-rest.md)
* [Параметры по умолчанию](/articles/es6-4-defaults.md)
* [Реструктуризующее присваивание (destructuring)](/articles/es6-6-destructuring.md)
* [Интерполяция и умные строки](/articles/es6-7-interpolation.md)
* [Стрелочные функции](/articles/es6-8-arrow-functions.md)
* [Цикл for .. of](/articles/es6-9-for-of-loop.md)
* [Расширение литерала объектов](/articles/es6-10-object-literal.md)

### Советы
* HTML
	* [Использование srcset для SVG изображений](/advices/srcset.md)
	* [Темы для мобильного Chrome](/advices/chrome-themes.md)
* CSS
	* [Наследование свойства box-sizing](/advices/box-sizing.md)
	* [Список, разделенный запятыми](/advices/css-lists.md)
	* [Отображение текста в пустых ссылках](/advices/empty-links.md)
	* [Ненужное свойство display: block](/advices/floats.md)
	* [Отладка лэйаутов с помощью универсального селектора](/advices/layout-debug.md)
	* [Применение стилей ко всем элементам, кроме последнего](/advices/not-last.md)
	* [Псевдоклассы :required и :optional](/advices/required-and-optional.md)
	* [Стилизация выделенного текста](/advices/selection.md)

* JavaScript
	* [Клонирование объекта](/advices/clone.md)
	* [Удаление всех ложных свойств из массива](/advices/filter-boolean.md)
	* [Поиск всех соседних элементов (siblings)](/advices/siblings.md)
	* [Передача аргументов в колбэки](/advices/callback-args.md)
	* [Простой chaining](/advices/chaining.md)
	* [Измерение производительности блоков кода](/advices/console-time.md)
	* [Проверка загрузки страницы](/advices/ready-state.md)

### Задачи
1. [Удаление всех гласных букв из строки](/exercises/remove-letters.md)
2. [Тире между двумя четными числами](/exercises/dashes.md)
3. [Землетрясение](/exercises/earthquake.md)
4. [Аккумуляция символов ](/exercises/accumulate.md)
5. [Вычисление остатка при делении](/exercises/remainder.md)
6. [Удаление элементов из массива по индексу](/exercises/remove-from-array.md)
7. [Объект наблюдатель](/exercises/observable.md)
8. [Найди анаграммы](/exercises/is-anagram.md)
9. [Смешай цвета](/exercises/mix-colors.md)
10. [Строковая шаблонизация](/exercises/template.md)
11. [Пара двойников](/exercises/doubles.md)
11. [Что за треугольник?](/exercises/triangles.md)
12. [Последовательность скобок](/exercises/brackets.md)

### Заметки
1. [Числа с плавающей точкой](/tests/numbers/dot.md)
2. [Унарный оператор +](/tests/numbers/plus_operator.md)
3. [Проверка на число](/tests/numbers/isNumber.md)
4. [NaN (not a number)](/tests/numbers/nan.md)
5. [Пустые ячейки в массиве](/tests/arrays/empty.md)
6. [Метод reduce](/tests/arrays/reduce.md)
7. [Hoisting: поднятие переменных вверх](/tests/functions/hoisting.md)
