...
description: "Наблюдатель (observer) — объект, поддерживающий подписки на обознченные события. Всё, что в будущем будет происходить с объектом как бы \"прослушивается\" и при выполнения определенного действия вызывается callback функция. Подобная событийная система встроена во многие библиотеки. В этой задачи разберём все основные концепции, а также некоторые баги, которые могут возникнуть при создании решения."
